...
LRESULT CALLBACK WindowProcedure (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    TCHAR szText3[] = TEXT("官方网站");
    TCHAR szText2[] = TEXT("dev.leyoblog.top/zzedubooster.html");
    ...
    case WM_PAINT:
        ...
        DrawText (hdc, TEXT ("\n \n郑州一中电阅助手\nDesigned By -翎风-"), -1, &rect, DT_CENTER) ;
        TextOut(hdc, 75, 415, szText, 17);
        TextOut(hdc, 17, 397, szText2, 34);
        TextOut(hdc, 104, 380, szText3, 8);
        EndPaint (hWnd, &ps) ;
        break ;
    case WM_CREATE:
  ...
  hwndButton4 = CreateWindow ( TEXT("button"), 
	...
	if (LOWORD(wParam) == 556 && HIWORD(wParam) == BN_CLICKED)
		{
			UINT starter = MessageBox(hWnd, TEXT("是否确定执行加速？注意！此功能仍在测试！"), TEXT("提示"), MB_ICONEXCLAMATION|MB_OKCANCEL);
			if(starter == IDOK) 
			{
			WinExec(dns.c_str(),SW_HIDE);
			MessageBox(hWnd, TEXT("加速完成！"), TEXT("提示"), MB_ICONEXCLAMATION|MB_OK);
			}
		}
			break; 
    case WM_DESTROY:
